...
const FlowchartDisplay = ({ process, onNodeClick, onUpdateNode, selectedNodeId }) => {
  ...
  return (
    <div className="w-full">
      {/* Show References Button - Only if there's actual reference data */}
      <div className="mb-4 flex items-center justify-between">
        ...
        {(() => {
          // Check if there's any meaningful reference data
          ...
          const hasRecoverySteps = (quickRef.recoverySteps || []).length > 0;
          const hasGaps = process?.gapAnalysis && Object.values(process.gapAnalysis).some(g => g?.length > 0);
          
          const hasAnyReferences = hasContacts || hasReferences || hasCriticalActions || hasTimings || hasRecoverySteps || hasGaps;
          
          // Only show button if there's actual data
          if (!hasAnyReferences) {
            return null;
          }

...
```bash
curl -X POST https://sop-genius-2.preview.emergentagent.com/api/process/analyze-document \
...
```bash
curl https://sop-genius-2.preview.emergentagent.com/api/learning/insights | python -m json.tool
```
...
curl -X POST https://sop-genius-2.preview.emergentagent.com/api/process/generate-from-analysis \
  -H "Content-Type: application/json" \
  -d '{

...
```bash
curl -X POST https://sopchart.preview.emergentagent.com/api/process/analyze-document \
...
```bash
curl https://sopchart.preview.emergentagent.com/api/learning/insights | python -m json.tool
```
...
curl -X POST https://sopchart.preview.emergentagent.com/api/process/generate-from-analysis \
  -H "Content-Type: application/json" \
  -d '{

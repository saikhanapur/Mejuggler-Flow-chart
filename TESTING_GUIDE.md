...
```bash
curl -X POST https://flowmapper-2.preview.emergentagent.com/api/process/analyze-document \
...
```bash
curl https://flowmapper-2.preview.emergentagent.com/api/learning/insights | python -m json.tool
```
...
curl -X POST https://flowmapper-2.preview.emergentagent.com/api/process/generate-from-analysis \
  -H "Content-Type: application/json" \
  -d '{
